...
: ${URL?"URL not set"}
[ -f /changelogs/changelog.* ] || (echo "Cannot find a single change log matching /changelogs/changelog.*" ; exit $ERROR_EXIT_CODE)

cat <<CONF > /home/rundir/liquibase.properties
...
while [  $COUNT -le $MAX_TRIES ]; do
   ...
   liquibase --changeLogFile="/changelogs/changelog.*" update
   if [ $? -eq 0 ];then
...
echo "Too many non-successful attempts"
